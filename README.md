...
3. **Контроль выполнения**
   Каждый тест выполняется внутри защищённого блока, построенного на `setjmp`/`longjmp`:
   мягкие проверки только копят сообщения, жёсткие — делают `longjmp` и мгновенно прерывают тест.
